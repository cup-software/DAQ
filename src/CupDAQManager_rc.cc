...
void CupDAQManager::RC_MERGER()
{
  ...
  fLog->Info("CupDAQManager::RC_MERGER", "ADCMode is %s",
             adcmode);  
...
void CupDAQManager::RC_STDDAQ()
{
  ...
  th2 = std::thread(&CupDAQManager::TF_WriteEvent, this);
  th3 = std::thread(&CupDAQManager::TF_BuildEvent, this);
  th4 = std::thread(&CupDAQManager::TF_SortEvent, this);
  ...
  while (true) {
    if (IsForcedEndRunFile() || fRunStatus == kERROR) {
      StopTrigger();
      time(&fEndDatime);
...
{
  int state = 0;
  fLog->Info("CupDAQManager::RC_NullTCB", "NULLTCB controller now starting [run=%d]",
             fRunNumber);
